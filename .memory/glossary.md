...
### `consent_status`
عمود في `leads.csv` — **الموافقة التسويقية** وحدها: إذن صريح يدوم حتى يُسحَب، وتشترطه واتساب لقالب خارج النافذة. قيمه اليوم: `none` (لم تُطلب ولم تُمنَح — كل صف يكتبه مسار حيّ) و`legacy_unknown` (صف ما قبل العمود، لم يُلاحَظ على هذا المحور). لا يقرأه شيء بعد. انظر D-021 وPRD §19.

### `contact_window_opened_at`
عمود في `leads.csv` — **طابع زمني** للحظة فتحت رسالتها نافذة الخدمة (≈24 ساعة يُسمح فيها بالرد بنص حر). ليس حالة: النافذة زمن ينقضي وحده. `""` = لا سجل (صف قديم، ولا يُشتق من وقت الإنشاء). القيمة **حدّ أدنى** للحظة الفتح الحقيقية. غير الموافقة تماماً، ولا يُقرأ أحدهما مكان الآخر.
